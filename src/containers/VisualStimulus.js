    ...
    for (var x = 0; x < w; x++) {
      for (var y = 0; y < h; y++) {
        if (this.props.showContrast) {
          //const val = (Math.abs(Math.floor((x - xOffset + l) / l) % 2) ^ Math.abs(Math.floor((y - yOffset) / l) % 2)) * 163; checkerboard original ACH on CCOPE
          const val = Math.abs(Math.floor(Math.sin(2 * 3.14 * (14 * x  - 14 * y)))) / 2 * 163; // diagonal stripes
          data[(x + y * w) * 4 + 0] = val*0.97224;
          data[(x + y * w) * 4 + 1] = val*0;
